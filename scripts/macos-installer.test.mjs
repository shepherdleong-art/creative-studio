...
const macElectronInstallCommand = build.indexOf('\n    if ! node "$ROOT/node_modules/electron/install.js"; then\n');
const macMissingRuntime = build.indexOf('\n  echo "Missing Electron runtime:');
assert.notEqual(macNpmCiCommand, -1);
assert.notEqual(macElectronInstallCommand, -1);
assert.notEqual(macMissingRuntime, -1);
assert.ok(macNpmCiCommand < macElectronInstallCommand, 'macOS 必须在 npm ci 后按需执行 Electron installer');
assert.ok(macElectronInstallCommand < macMissingRuntime, 'macOS 必须在 Electron installer 后硬失败检查 runtime');
const macNpmCiBlockStart = build.indexOf('if [ "$SKIP_NPM_CI" -eq 1 ]; then');
const macNpmCiElse = build.indexOf('\nelse\n', macNpmCiBlockStart);
const macNpmCiBlockEnd = build.indexOf('\nfi\n\nif [ ! -d "$ELECTRON_APP" ]', macNpmCiElse);
assert.ok(macNpmCiBlockStart >= 0 && macNpmCiElse > macNpmCiBlockStart && macNpmCiBlockEnd > macNpmCiElse);
assert.doesNotMatch(build.slice(macNpmCiBlockStart, macNpmCiElse), /electron\/install\.js/, 'macOS skip 分支不得自动安装 Electron');
assert.match(build.slice(macNpmCiElse, macNpmCiBlockEnd), /npm ci[\s\S]*if \[ ! -d "\$ELECTRON_APP" \][\s\S]*node "\$ROOT\/node_modules\/electron\/install\.js"/);
