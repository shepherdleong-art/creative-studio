...
const narrationGainControl = fs.readFileSync('components/mixcut/NarrationGainControl.tsx', 'utf8');
...
assert.match(panel, /isMixcutScriptChoiceVisible/, '脚本可见性必须复用统一纯函数，不得各处自写等值过滤');
assert.doesNotMatch(panel, /draft\.shotSetId === (?:activeShotSetId|currentShotSetId)/, '不得再用精确分镜组等值过滤脚本');
assert.match(panel, /setGroupScriptSource/, '已有混剪必须记录源脚本 revision 身份用于新版本提示');
assert.match(panel, /syncScriptToRevision/, '同步最新版本必须通过显式编辑器状态迁移，不得静默覆盖');
assert.match(panel, /sourceScriptUpdate=/, 'CreationStep 必须接收源脚本新版本状态');
assert.match(creationStep, /源脚本有新版本/, '源脚本有新 revision 时必须显示提示');
assert.match(creationStep, /同步最新版本/, '必须提供显式同步入口');
assert.match(creationStep, /data-testid="mixcut-source-script-update"/, '新版本提示必须有稳定验证入口');
assert.match(creationStep, /保留修改并同步/, '手改文案时同步必须提供保留/放弃/取消三路决策');
assert.match(creationStep, /放弃修改并同步/);
assert.match(creationStep, /draftMetaLabel/, '项目脚本与历史脚本必须走统一元数据文案');
assert.match(creationStep, /当前版本 V\$\{/, '项目脚本必须显示当前版本 Vn，不得显示空 provider/model 斜杠');
assert.doesNotMatch(creationStep, /仅显示当前分镜组的有效草稿/, '文案必须表达「项目脚本 + 当前组历史脚本」');
assert.match(creationStep, /项目脚本与当前分镜组的历史脚本/, '下拉说明必须包含项目级脚本');
