...
assert.match(main, /return \{ requestId, count \}/);
assert.match(main, /filePaths: \[filePaths\[index\]\]/);
assert.match(main, /linkedImportProgress/);
assert.match(main, /completed: index \+ 1/);
assert.match(main, /total: filePaths\.length/);
...
assert.match(main, /relocateLinkedSource/);
...
assert.match(service, /electron-service\.json/);
assert.match(service, /persistServiceState/);
assert.match(service, /clearServiceState/);
