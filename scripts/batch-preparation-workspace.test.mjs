...
assert.match(materials, /自定义导入素材/);
assert.match(materials, /onImportFiles/);
assert.match(preparation, /splitBatchRenderTasks/);
assert.match(preparation, /第 4 步导出时进行/);
...
assert.doesNotMatch(preparation, /渲染成片/, '第 2 步进度卡不展示整片渲染阶段');
assert.doesNotMatch(preparation, /生成封面/, '第 2 步进度卡不展示封面生成阶段');
// 第 3 步:检查页 = 封面墙 + 点卡片直接进编辑器(编辑器优先,无历史版本入口/双模式)
assert.doesNotMatch(review, /只重新分配这一条/);
assert.match(review, /换一批画面/);
assert.match(review, /重试封面/);
assert.match(review, /重试配音/);
assert.doesNotMatch(review, /另有 .* 个历史版本/, '历史版本入口必须移除');
assert.doesNotMatch(review, /版本切换/);
assert.doesNotMatch(review, /查看版本/);
assert.doesNotMatch(review, /待重新生成|等待重新渲染/, '待重渲文案必须移除(重试配音里的“重新生成自动字幕”是口播文案,保留)');
assert.doesNotMatch(review, /重试渲染/);
