      ...
      <div className={styles.inspectorScroll}>
        {mode === 'subtitle' && (
          <>
            ...
            <StyleEditor key={`subtitle-${group.revision}`} value={group.textStyles[variant.outputPreset].subtitle} onCommit={(style) => onGroupCommand({ type: 'set_text_style', preset: variant.outputPreset, target: 'subtitle', style })} />
          </>
        )}
        ...
        {mode === 'cover' && (
          <>
            ...
            <Slider label="缩放" value={variant.cover.framing.scale} min={1} max={3} step={0.05} onCommit={(scale) => onVariantCommand({ type: 'set_cover_framing', scale, offsetX: variant.cover.framing.offsetX, offsetY: variant.cover.framing.offsetY })} />
            <Slider label="水平偏移" value={variant.cover.framing.offsetX} min={-1} max={1} step={0.05} onCommit={(offsetX) => onVariantCommand({ type: 'set_cover_framing', scale: variant.cover.framing.scale, offsetX, offsetY: variant.cover.framing.offsetY })} />
            <Slider label="垂直偏移" value={variant.cover.framing.offsetY} min={-1} max={1} step={0.05} onCommit={(offsetY) => onVariantCommand({ type: 'set_cover_framing', scale: variant.cover.framing.scale, offsetX: variant.cover.framing.offsetX, offsetY })} />
            ...
            <StyleEditor key={`${styleTarget}-${group.revision}`} value={group.textStyles[variant.outputPreset][styleTarget === 'subtitle' ? 'coverPrimary' : styleTarget]} onCommit={(style) => onGroupCommand({ type: 'set_text_style', preset: variant.outputPreset, target: styleTarget === 'subtitle' ? 'coverPrimary' : styleTarget, style })} />
        ...
        {mode === 'framing' && (
          <>
            ...
            {selectedClip ? (
              <>
                <Slider label="缩放" value={selectedClip.framing.scale} min={1} max={3} step={0.05} onCommit={(scale) => onVariantCommand({ type: 'set_framing', clipId: selectedClip.id, scale, offsetX: selectedClip.framing.offsetX, offsetY: selectedClip.framing.offsetY })} />
                <Slider label="水平偏移" value={selectedClip.framing.offsetX} min={-1} max={1} step={0.05} onCommit={(offsetX) => onVariantCommand({ type: 'set_framing', clipId: selectedClip.id, scale: selectedClip.framing.scale, offsetX, offsetY: selectedClip.framing.offsetY })} />
                <Slider label="垂直偏移" value={selectedClip.framing.offsetY} min={-1} max={1} step={0.05} onCommit={(offsetY) => onVariantCommand({ type: 'set_framing', clipId: selectedClip.id, scale: selectedClip.framing.scale, offsetX: selectedClip.framing.offsetX, offsetY })} />
        ...
        {mode === 'audio' && (
          <>
            ...
            <Slider label="BGM 增益（dB）" value={variant.bgm.gainDb} min={-40} max={0} step={0.5} onCommit={(gainDb) => onVariantCommand({ type: 'set_bgm_gain', gainDb })} />
...
function StyleEditor({ value, onCommit }: { value: TextStyle; onCommit: (style: TextStyle) => void }) {
  ...
  const commit = (patch: Partial<TextStyle>) => { const next = { ...draft, ...patch }; setDraft(next); onCommit(next); };
  ...
  return (
    <div>
      ...
      <Slider label="字号" value={draft.fontSizePx} min={12} max={180} step={1} onCommit={(fontSizePx) => commit({ fontSizePx })} />
      <Slider label="X 位置" value={draft.x} min={0} max={1} step={0.01} onCommit={(x) => commit({ x })} />
      <Slider label="Y 位置" value={draft.y} min={0} max={1} step={0.01} onCommit={(y) => commit({ y })} />
      <Slider label="缩放" value={draft.scale} min={0.5} max={2} step={0.01} onCommit={(scale) => commit({ scale })} />
      <Slider label="单行宽度" value={draft.boxWidthPx} min={200} max={1800} step={10} onCommit={(boxWidthPx) => commit({ boxWidthPx })} />
      ...
      {draft.stroke.enabled && <div className={styles.effectBody}><label className={styles.colorRow}><input type="color" value={draft.stroke.color} onChange={(event) => commit({ stroke: { ...draft.stroke, color: event.target.value } })} /><span /></label><Slider label="粗细" value={draft.stroke.widthPx} min={0} max={16} step={0.5} onCommit={(widthPx) => commit({ stroke: { ...draft.stroke, widthPx } })} /></div>}
      ...
      {draft.shadow.enabled && <div className={styles.effectBody}><label className={styles.colorRow}><input aria-label="阴影颜色" type="color" value={draft.shadow.color} onChange={(event) => commit({ shadow: { ...draft.shadow, color: event.target.value } })} /><span /></label><Slider label="不透明度" value={draft.shadow.opacity} min={0} max={1} step={0.05} onCommit={(opacity) => commit({ shadow: { ...draft.shadow, opacity } })} /><Slider label="模糊" value={draft.shadow.blurPx} min={0} max={40} step={1} onCommit={(blurPx) => commit({ shadow: { ...draft.shadow, blurPx } })} /><Slider label="距离" value={draft.shadow.distancePx} min={0} max={40} step={1} onCommit={(distancePx) => commit({ shadow: { ...draft.shadow, distancePx } })} /><Slider label="角度" value={draft.shadow.angleDeg} min={0} max={360} step={1} onCommit={(angleDeg) => commit({ shadow: { ...draft.shadow, angleDeg } })} /></div>}
    </div>
  );
}

function Slider({ label, value, min, max, step, onCommit }: { label: string; value: number; min: number; max: number; step: number; onCommit: (value: number) => void }) {
  return <label className={styles.sliderField} key={`${label}-${value}`}><span>{label}</span><div className={styles.sliderRow}><input type="range" defaultValue={value} min={min} max={max} step={step} onPointerUp={(event) => onCommit(Number(event.currentTarget.value))} /><input type="number" defaultValue={value} min={min} max={max} step={step} onBlur={(event) => onCommit(Number(event.currentTarget.value))} /></div></label>;
}
