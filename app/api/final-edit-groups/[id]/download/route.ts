...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    for (const variant of variants) {
      ...
      if (!row) continue;
      const output = JSON.parse(row.outputJson) as { videoRelativePath: string; coverRelativePath: string };
      const prefix = `成片-${String(variant.indexNum).padStart(2, '0')}`;
      entries.push({ filePath: path.join(dataRoot(), 'storage', output.videoRelativePath), filename: `${prefix}/${prefix}.mp4` });
      entries.push({ filePath: path.join(dataRoot(), 'storage', output.coverRelativePath), filename: `${prefix}/${prefix}-封面.jpg` });
    }
