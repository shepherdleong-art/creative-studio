...
/** 读取一段字节范围(或整段小文件),支持 <video> 拖动所需的 Range 请求。 */
function serveFile(request: NextRequest, absolutePath: string, extraHeaders: Record<string, string>): NextResponse {
  ...
  if (rangeHeader) {
    ...
    if (!match[1]) {
      const suffixLen = Number.parseInt(match[2], 10);
...
export async function GET(request: NextRequest, context: { params: Promise<{ assetId: string }> }) {
  ...
  try {
    ...
    if (source.kind === 'original') {
      ...
      return serveFile(request, source.sourcePath, { 'X-Preview-Kind': 'original' });
    }
    if (source.kind === 'original_pending_lut') {
      if (!fs.existsSync(source.sourcePath)) {
        return NextResponse.json({
      ...
      return serveFile(request, source.sourcePath, {
        'X-Preview-Kind': 'original_pending_lut',
        'X-Preview-Warning': encodeURIComponent(source.warning),
      });
    }
    // source.kind === 'proxy'
