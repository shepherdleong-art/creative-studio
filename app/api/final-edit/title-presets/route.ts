...
export async function GET() {
  ...
  return NextResponse.json(rows.map((row) => ({ id: row.id, name: row.name, stylesByPreset: sanitizeTitlePresetStyles(JSON.parse(row.stylesByPresetJson)), createdAt: row.createdAt, updatedAt: row.updatedAt })));
}
export async function POST(request: Request) {
  try {
    ...
    const styles = sanitizeTitlePresetStyles(body.stylesByPreset);
    const id = uuidv4(); const timestamp = new Date().toISOString();
    getDb().prepare(`INSERT INTO final_edit_title_presets (id, name, stylesByPresetJson, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?)`).run(id, name, JSON.stringify(styles), timestamp, timestamp);
    return NextResponse.json({ id, name, stylesByPreset: styles, createdAt: timestamp, updatedAt: timestamp }, { status: 201 });
