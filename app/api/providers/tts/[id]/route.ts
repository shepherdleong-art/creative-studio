...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const current = getDb().prepare(`SELECT * FROM final_edit_tts_providers WHERE id=?`).get(id) as { baseUrl: string; apiKey: string; enabled: number; costPerThousandCharacters: number } | undefined;
  ...
  const cost = body.costPerThousandCharacters == null ? current.costPerThousandCharacters : Number(body.costPerThousandCharacters);
