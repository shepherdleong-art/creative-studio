...
- **`lib/`** — Business logic
  ...
  - `schema-upgrade/` — shared safe-upgrade infrastructure: SQLite Online Backup, disk preflight, a cross-process SQLite write lock, and durable JSONL audit records; paths derive from `dataRoot()`
  - `batch-production/` — independent batch-production module; `schema.ts` uses versioned `IMMEDIATE` transactions and `readiness.ts` is the locked, audited gate behind `GET /api/batch-production/readiness`. If unavailable, disable only batch production and preserve legacy projects and single precise mixcut
