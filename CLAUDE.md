...
Company-gateway local stack (Windows): in development, when a runtime (`.venv-litellm` or `.litellm-runtime`) and `config.yaml` are present, `start-windows.cmd` first runs `scripts/start-stack.ps1 -SkipApp` to bring up the LiteLLM proxy (port 4000, bound to 127.0.0.1), then starts the dev server. `scripts/setup-company-gateway.ps1` assembles `.litellm-runtime` with one command (pinned private CPython 3.12.10 + LiteLLM 1.89.2, no Python install required; identical to the installer-bundled sidecar); sidecar launches always set `LITELLM_LOCAL_MODEL_COST_MAP=True` so the remote cost-map fetch cannot stall startup on corporate networks. The installed app bundles a pinned private CPython/LiteLLM sidecar and accepts a first-run AES-256-GCM `.provision` package; re-import rotates the managed provider keys. The administrator workflow is documented in `provisioning/README.md`. Public delivery of reference images goes through Tencent COS (`CREATIVE_STUDIO_COS_*`, see `lib/cos-media.ts`). Local services must never be public. Stop commands and `/api/shutdown` use the controlled `storage/run/stack.json` stop script. The state file is BOM-less JSON; every `scripts/*.ps1` and `installer/windows/*.ps1` file MUST stay UTF-8 **with BOM** for PowerShell 5.1.
