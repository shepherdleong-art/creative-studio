...
- **`lib/`** — Business logic
  ...
  - `batch-production/` — independent batch-production module; `schema.ts` uses versioned `IMMEDIATE` transactions and `readiness.ts` is the locked, audited gate behind every batch API. Published migrations v1–v9 cover the batch domain foundation; v10 adds authoritative per-source rows, v11 adds structured script/snapshot metadata, and v12 tracks upstream availability plus catalog ownership. Rows proven to come from catalog sync (by revision metadata or a currently present upstream draft) retire when invalid/deleted; older ambiguous rows are conservatively retained so independently saved project scripts and frozen snapshots are never guessed away. Source parsing remains compatible with v10 location JSON that predates the `kind` discriminator, including legacy managed paths rooted at `dataRoot()/storage/batch-media`. `media-catalog.ts` trusts authoritative `video_jobs` rows, validates project/shot-set ownership, controlled paths, real video containers, and full SHA-256, and relinks one explicit linked-source id only after an exact-content match. `prepare.ts` runs behind the readiness gate and `MixcutWorkspace.tsx` exposes single and batch modes. Phase B atomically confirms a draft whole-input selection in `batch-flow.ts`; identical input is idempotent and only changed input creates a revision. `startBatchProduction` syncs the latest project scripts, verifies an explicit analyzed-media pool and exactly N plans, then permanently freezes the version in one transaction. The fifth step can create/select a batch, set per-script copy counts, select analyzed media, inspect N output cards, restore them after refresh, and start. Batch APIs are `POST/GET /api/batch-production/batches`, `GET /api/batch-production/batches/[id]`, `POST .../[id]/snapshot`, and `PUT .../[id]/start`. Phase C (`scheduler.ts`) adds atomic claim, finite leases, control-aware stale/startup recovery, retry, and pause/resume/stop; `executors.ts` defines the task-execution Adapter seam with honest progress (unknown phases report null percent); `runner.ts` provides the claim-execute-complete loop and a process singleton that distinguishes recoverable application shutdown from irreversible user stop. v13 adds lease fields and `interrupted` to attempts, `requestKey`/`expectedState` to tasks, and `controlState` to batches. Node startup resumes scheduling through readiness-gated `instrumentation.ts`, with idempotent fallback from start/task APIs. Progress/control APIs: `GET .../[id]/tasks`, `POST .../[id]/control`, and `POST /api/batch-production/tasks/[taskId]/retry`. Public domain interfaces live in `assets.ts`/`versions.ts`/`scripts.ts`/`plans.ts`/`tasks.ts`/`artifacts.ts`. `GET /api/batch-production/recovery` only lists and revalidates candidates; no running API may overwrite the live database. If unavailable, disable only batch production and preserve legacy projects and single precise mixcut
