...
- **`lib/`** — Business logic
  ...
  - `batch-production/` — independent batch-production module; `schema.ts` uses versioned `IMMEDIATE` transactions and `readiness.ts` is the locked, audited gate behind `GET /api/batch-production/readiness`. Published migrations v1–v7 cover the full batch domain: project assets + analysis versions (v2, identity by content fingerprint, never by path), batch versions + asset pools (v3), project scripts + snapshots (v4, snapshots never drift after a batch starts), output plans + output versions (v5, copy count decides exactly N plans), tasks + attempts (v6, retries only add attempts), artifacts + current-output pointer (v7, artifacts append-only). Public domain interfaces live in `assets.ts`/`versions.ts`/`scripts.ts`/`plans.ts`/`tasks.ts`/`artifacts.ts`. `GET /api/batch-production/recovery` only lists and revalidates candidates; no running API may overwrite the live database. If unavailable, disable only batch production and preserve legacy projects and single precise mixcut
