...
### 6.5 素材替换列（左，对标 MaterialReplaceList）
- 面板标题「素材替换」（refresh 图标）+ 橙色引导文案「先在时间线选中片段，再点下面素材直接替换；仅显示本次参与混剪的素材。」
- 行：38×54 竖版缩略图 + 名称 + 时长·分辨率（等宽小字）+ 「已用」绿 chip；点击=替换当前选中片段（样机里换缩略图演示）。排除的素材不出现在此列。
...
- [ ] 双击片段开 Trim、拖动选择框、完成/取消收回；点击素材替换行替换选中片段缩略图。
