...
- **口播音频倍速**：口播音频轨右键打开横向拉条 + 右侧数值框，范围 0.5x～2.0x、步进 0.1x；拉条与数值框双向同步，调节过程只预览数值，点击确认后才保留当前版本并按所选倍速生成独立新版本，不得拖动一次就连续创建任务，也不伪装成仅改变播放器速度。
...
- [ ] 右键口播音频轨显示 0.5x～2.0x 拉条和同步数值框；拖动不提交，确认后才生成独立新版本。
